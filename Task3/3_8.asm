...
; Tim tong gia tri chan, tong gia tri le
outputsize = 13
.data
n db 2 dup(0)
count dd 0
...
evenn dd 0            ; Tong chan
oddd dd 0            ; Tong le
string1 db "Tong so chan "
string2 db "Tong so le "
tongchan db 0,0,0,0,0,0ah
tongle db 0,0,0,0,0,0ah
.code
strtoi proc
L1:
...
itostr proc     ; ham doc gia tri trong esi va bien no thanh string 5 chu so
; luu trong array [esi]
mov cx, 10      ; thanh ghi 32bit

div cx  ; chia eax cho 10
add dl, 30h         ; dx luu remnant
...
lap:
    cmp ecx, count
    jz endi         ; da duyet het mang, thoat
    mov al, BYTE PTR [esi + ecx * 4]  ; thanh ghi min
    mov bl, 1
    and bl, al
    jz congchan ; zero thi la so chan
    ; cong le
    add oddd, eax
    inc ecx
    jmp lap
congchan:
    ...
    jmp lap
endi:
mov esi, offset tongchan
mov eax, evenn
call itostr
mov esi, offset tongle
...
invoke WriteConsole, ; goi WriteConsole de thuc hien output
       HandleWrite,   ; Handle de thuc hien lenh
       ADDR tongchan,    ; Dia chi cua mang can output
...
invoke WriteConsole, ; goi WriteConsole de thuc hien output
       HandleWrite,   ; Handle de thuc hien lenh
       ADDR tongle,    ; Dia chi cua mang can output
       6,    ; Do dai cua du lieu output
       ADDR realout,  ; Do dai thuc su cua du lieu output
       0              ; Ket thuc doc
